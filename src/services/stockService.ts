...
export const getStockTransactions = async (): Promise<ApiResult> => {
    try {
        ...
        return {
            success: false,
            message: error instanceof Error ? error.message : 'Unable to load stock transactions.',
        };
    }
};
...
export const getStockTransactionById = async (id: string): Promise<ApiResult> => {
    try {
        const response = await fetch(`${BASE_API}/stock/${id}`, {
            headers: await getHeaders(),
            cache: 'no-store',
        });
        return (await response.json()) as ApiResult;
        ...
        return {
            success: false,
            message: error instanceof Error ? error.message : 'Unable to load stock transaction.',
        };
    }
};
...
export const createStockTransactions = async (stockData: FieldValues): Promise<ApiResult> => {
    try {
        ...
        return {
            success: false,
            message: error instanceof Error ? error.message : 'Unable to create stock transaction.',
        };
    }
};
...
export const updateStockTransaction = async (
    ...
    try {
        ...
        return {
            success: false,
            message: error instanceof Error ? error.message : 'Unable to update stock transaction.',
        };
    }
};
