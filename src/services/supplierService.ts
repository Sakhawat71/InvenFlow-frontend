...
export const getSuppliers = async (): Promise<ApiResult> => {
    try {
        const response = await fetch(`${BASE_API}/supplier`, {
        ...
        return {
            success: false,
            message: error instanceof Error ? error.message : 'Unable to load suppliers.',
        };
    }
};
...
export const getSupplierById = async (id: string): Promise<ApiResult> => {
    try {
        const response = await fetch(`${BASE_API}/supplier/${id}`, {
            headers: await getHeaders(),
            cache: 'no-store',
        });
        return (await response.json()) as ApiResult;
...
export const updateSupplier = async (id: string, supplierData: FieldValues): Promise<ApiResult> => {
    try {
        ...
        return {
            success: false,
            message: error instanceof Error ? error.message : 'Unable to update supplier.',
        };
    }
};
