...
export const getPurchaseOrders = async (): Promise<ApiResult> => {
    try {
        const response = await fetch(`${BASE_API}/purchase-order`, {
...
export const getPurchaseOrderById = async (id: string): Promise<ApiResult> => {
    try {
        const response = await fetch(`${BASE_API}/purchase-order/${id}`, {
            headers: await getHeaders(),
            cache: 'no-store',
        });
        return (await response.json()) as ApiResult;
        ...
        return {
            success: false,
            message: error instanceof Error ? error.message : 'Unable to load purchase order.',
        };
    }
};
...
export const createPurchaseOrder = async (orderData: FieldValues): Promise<ApiResult> => {
    try {
        ...
        return {
            success: false,
            message: error instanceof Error ? error.message : 'Unable to create purchase order.',
        };
    }
};
...
export const updatePurchaseOrder = async (
    ...
    try {
        ...
        return {
            success: false,
            message: error instanceof Error ? error.message : 'Unable to update purchase order.',
        };
    }
};
...
export const deletePurchaseOrder = async (id: string): Promise<ApiResult> => {
    try {
        ...
        return {
            success: false,
            message: error instanceof Error ? error.message : 'Unable to delete purchase order.',
        };
    }
};
