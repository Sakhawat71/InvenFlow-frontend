...
export const getReports = async (): Promise<ApiResult> => {
  try {
    ...
    return { success: false, message: error instanceof Error ? error.message : "Unable to load reports." };
  }
};
...
export const getReportByType = async (reportType: string): Promise<ApiResult> => {
  try {
    ...
    return { success: false, message: error instanceof Error ? error.message : "Unable to load report." };
  }
};
